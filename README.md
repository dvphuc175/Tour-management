...
- **Bảo mật:** middleware CSRF tự viết (token + `timingSafeEqual`), `sanitize-html` chống XSS cho rich text, cookie HttpOnly/SameSite.
- **Validation:** Joi 18
- **Upload ảnh:** `multer` + `multer-storage-cloudinary` lưu lên Cloudinary CDN.
- **Thanh toán:** VNPay sandbox (SDK `vnpay@^2.5.0`, sử dụng Return URL — chưa có IPN webhook).
...
- **Tài liệu:** SRS, SDD, Chương 5 Thiết kế giao diện (đính kèm bộ wireframe HTML lo-fi).
...
└── database/
    ...
    └── Truncate_data.sql       # Xóa toàn bộ dữ liệu (giữ schema)
```
...
Nhóm áp dụng mô hình Agile (Scrum) và quy ước Git cơ bản:
...
2. Tạo nhánh tính năng mới theo quy ước `feature/<tên-chức-năng>` hoặc `fix/<mô-tả>`:

   ```bash
   git checkout -b feature/booking-cancel
   ```

3. Commit với thông điệp rõ ràng, ngắn gọn (tiếng Việt hoặc tiếng Anh đều được, miễn là thống nhất):

   ```bash
   git commit -m "feat(booking): cho phép khách tự hủy đơn pending"
   ```

4. Push lên remote và tạo Pull Request (PR) vào nhánh `main` để review trước khi merge.

5. KHÔNG commit file `.env` hoặc bất kỳ thông tin nhạy cảm nào lên repo.
...
- **SDD** — Tài liệu thiết kế kèm:
  - Sơ đồ kiến trúc, DFD, ERD.
  - Đặc tả Use Case.
  - **Chương 5 – Thiết kế giao diện**: sitemap, design system, bộ 20 wireframe lo-fi (HTML).
- **Báo cáo đối chiếu code ↔ SRS, code ↔ SDD** (kèm danh sách use case thiếu).

---

## Đội Ngũ Phát Triển

Dự án được phát triển bởi nhóm sinh viên chuyên đề. Cập nhật thông tin thành viên trước khi nộp:

- **Đỗ Văn Phúc** — *(Trưởng nhóm, fullstack)*
- **Cao Văn Quyết** — *(Thành viên)*
- *(Bổ sung thành viên khác nếu có)*

**Giảng viên hướng dẫn:** *(điền tên giảng viên)*
*Trường Đại học … | Hà Nội, Năm 2026*
